...
Two distinct sets of weights appear in this project, and confusing them would misread the tables below.
...
Every figure and table in this supplement is generated from the ten retrained checkpoints or from measurements on the recorded audio itself, except where a caption says otherwise.
...
Fig.~\ref{fig:esm-seed-spread} places all ten checkpoints on the five recorded held-out datasets. The vertical extent of each cluster, not its mean, is the object of the main article: on \texttt{thinkpad-2} the ten checkpoints run from 0.348 to 0.963.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{esm-seed-spread-cnn.pdf}
\caption{Accuracy of every reported checkpoint on the five recorded held-out datasets. Dashes are per-variant means; the cross is the earlier single checkpoint of Section~\ref{sec:provenance}.}
...
Both impulse-response banks convolve the evaluation waveform before CQT extraction, one response drawn per clip, with a fixed draw seed. Nothing is written back into training, and the overlays are applied to the five held-out datasets only.
...
The recorded datasets are at \url{https://zenodo.org/records/21077144} and the recording tool at \url{https://github.com/sglkc/chords-dataset-generator/}. Every per-checkpoint quantity tabulated above --- the training record, the per-epoch curves, the recorded and overlay accuracies, and the per-class recall of all ten checkpoints --- is reproducible from those datasets under the settings given in this section.
