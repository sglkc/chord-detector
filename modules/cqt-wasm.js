/**
 * CQT Feature Extraction Module (librosa-compatible, WebAssembly)
 * 
 * This is a wrapper that imports the WASM-based CQT implementation
 * from the librosa-cqt subproject for high-performance extraction.
 * 
 * The API is compatible with the pure JavaScript cqt-librosa.js module.
 */

// Import the WASM-based implementation
// Note: The path is relative to where this module is loaded from (index.html or app.js)
let wasmModule = null;
...
async function loadWasmModule() {
  ...
  loadPromise = (async () => {
    try {
      ...
      // Path relative to the HTML page that loads this module
      const wasm = await import('../librosa-cqt/build/release.js');
      wasmModule = wasm;
      wasmLoaded = true;
      console.log('CQT WASM module loaded successfully');
      return wasmModule;
    } catch (error) {
      console.error('Failed to load CQT WASM module:', error);
      throw error;
    }
  })();
 ...
 * API-compatible with cqt-librosa.js
 */
export class CQTExtractor {
  ...
  async init(options = {}) {
    ...
    // Check if already initialized with same config - skip re-initialization
    if (this.initialized && this.config &&
      this.config.sampleRate === sampleRate &&
      this.config.fmin === fmin &&
      this.config.nBins === nBins &&
      this.config.binsPerOctave === binsPerOctave &&
      this.config.hopLength === hopLength) {
    ...
    // Reset allocated pointers when config changes (WASM will reallocate)
    this.audioPtr = 0;
    this.outputPtr = 0;
    this.framePtr = 0;
    this.allocatedAudioLength = 0;
  ...
  async extractFeatures(windowData, config) {
    ...
    // Print features for verification (optional - can be disabled in production)
  ...
  extractFullCQT(audioData, options = {}) {
    if (!this.initialized) {
      // Synchronous fallback - init should have been called already
      console.warn('CQT not initialized, using default config');
      this.config = {
        sampleRate: options.sampleRate || 48000,
        fmin: options.fmin || 130.81,
        nBins: options.nBins || 36,
        binsPerOctave: options.binsPerOctave || 12,
...
// Named export for compatibility with cqt-librosa.js
